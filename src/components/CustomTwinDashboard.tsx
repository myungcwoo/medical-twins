...
export const CustomTwinDashboard: FC = () => {
  const {
      customTwins, isCustomRunning, isCustomEnded, customTicks, isCustomFastForwarding,
      handleCustomFastForward: onFastForward, setIsCustomRunning, setIsCustomEnded,
      setCustomTwins, setCustomTicks
  } = useSimulationStore();

  const onTogglePlay = () => setIsCustomRunning(!isCustomRunning);
  const onEndTrial = () => { setIsCustomRunning(false); setIsCustomEnded(true); };
  const onResetTrial = () => {
      setCustomTwins([]); setIsCustomEnded(false); setIsCustomRunning(false);
      setCustomTicks(0);
  };
  ...
  const renderPopulationSurvivalHeatmap = () => {
    ...
    return <PopulationHeatmap agents={customTwins} ticks={customTicks} />;
  };
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1rem' }}>
         <div style={{ background: 'rgba(0,0,0,0.3)', padding: '1.5rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '1.5rem' }}>
            ...
            <div style={{ width: '100%', height: 450 }}>
               <SurvivalComposedChart chartMode={chartMode} data={getCohortData()} />
            </div>
         </div>
      </div>
