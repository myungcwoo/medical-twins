...
export const KnowledgeGraphCanvas: React.FC<KnowledgeGraphProps> = ({
    ...
    return (
        <div style={{ flexShrink: 0, width: '280px', height: '280px', borderRadius: '16px', overflow: 'hidden', border: '1px solid rgba(139, 92, 246, 0.3)', boxShadow: '0 0 40px rgba(139, 92, 246, 0.15)', position: 'relative', background: '#000' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 50, display: 'flex', background: 'rgba(0,0,0,0.8)' }}>
                <button 
            ...
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '2rem 1rem 1rem 1rem', background: 'linear-gradient(to top, rgba(0,0,0,0.95) 20%, transparent)', pointerEvents: 'none', zIndex: 50 }}>
                <div style={{ color: '#8b5cf6', fontSize: '0.9rem', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Biological Hologram</div>
                <div style={{ color: '#e2e8f0', fontSize: '0.75rem', marginTop: '0.4rem', lineHeight: 1.4, opacity: 0.9 }}>
                    {mode === 'Topology' 
                        ? 'Maps real-time peer-to-peer influence. Glowing paths show where agents share the exact same clinical protocols.'
                        : 'Groups theoretical protocols. Nodes cluster tightly when interventions share high semantic similarity vectors.'}
                </div>
            </div>
        </div>
    );
