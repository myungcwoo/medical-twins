...
import { InSilicoTrialSandbox } from './InSilicoTrialSandbox';
...
export const LiteratureBoard: FC = () => {
    ...
    const handleRefresh = async () => {
        ...
        try {
            ...
            const newTrial = await LLMEngine.generateTrialAsync(searchQuery);
            
            if (newTrial.id && newTrial.title) {
                // Hold in pending state for sandbox validation
                setPendingTrial(newTrial);
                toast.success(`Extracted protocol. Ready for validation.`, { id: 'trial-sync' });
    ...
    return (
        <div style={{ padding: '2rem', animation: 'fadeIn 0.5s ease-out', maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', marginBottom: '2.5rem' }}>
                <div>
                    <h2 style={{ color: '#60a5fa', margin: '0 0 0.5rem 0', fontSize: '2rem', display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                        <span>📚</span> Protocol Discovery Dashboard
                    </h2>
                ...
                <div style={{ background: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.3)', padding: '1.5rem', borderRadius: '8px', display: 'flex', flexDirection: 'column', flex: '1 1 350px', alignItems: 'stretch' }}>
                    <div style={{ color: '#93c5fd', fontSize: '0.85rem', marginBottom: '0.8rem', textAlign: 'center' }}>Real-Time Evidence Extraction</div>
                    <input 
                        type="text" 
                        placeholder="Search disease or intervention (e.g. CRISPR for CKD)..."
                        value={searchQuery}
                        onChange={e => setSearchQuery(e.target.value)}
                        style={{ background: 'rgba(0,0,0,0.3)', color: 'white', border: '1px solid rgba(255,255,255,0.2)', padding: '0.8rem', borderRadius: '6px', marginBottom: '1rem', width: '100%' }}
                        disabled={isRefreshing || pendingTrial}
                    />
                    <button 
                        onClick={handleRefresh}
                        disabled={isRefreshing || pendingTrial !== null}
                        style={{ 
                            ...
                            borderRadius: '6px', 
                            fontWeight: 'bold', 
                            cursor: isRefreshing || pendingTrial !== null ? 'not-allowed' : 'pointer',
                            display: 'flex',
                            justifyContent: 'center'
                        }}
                    >
                        {isRefreshing ? <span style={{ animation: 'spin 1s linear infinite' }}>🔄</span> : <span>✨</span>}
                        {isRefreshing ? 'Extracting Trials...' : 'Simulate Literature Extraction'}
                    </button>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.8rem', textAlign: 'center' }}>Status: RAG Agent Active</div>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                {pendingTrial && (
                    <InSilicoTrialSandbox 
                        pendingProtocol={pendingTrial} 
                        onCancel={() => setPendingTrial(null)} 
                        onValidate={(template) => {
                            KnowledgeBase.CLINICAL_IDEAS.push(template);
                            setLiteratureDB(prev => [pendingTrial, ...prev]);
                            setPendingTrial(null);
                            toast.success(`Protocol validated & broadcasted to global network!`, { id: 'validate' });
                        }} 
                    />
                )}
