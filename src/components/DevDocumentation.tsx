    ...
    cacScore: number;            // [NEW] Calcium Plaque Score (0 to > 1000)
            ...
            <section style={{ marginBottom: '4rem' }}>
                <h2 style={{ color: '#f59e0b', borderBottom: '2px solid #78350f', paddingBottom: '0.5rem', fontSize: '1.8rem', display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                    <span role="img" aria-label="computer">💻</span> 4. Deep Learning Decoupling (Browser Edge & ONNX)
                </h2>
                <div style={{ background: 'rgba(0,0,0,0.4)', padding: '2rem', borderRadius: '12px', border: '1px solid rgba(245, 158, 11, 0.2)' }}>
                    <p style={{ marginTop: 0, lineHeight: '1.6', color: '#e2e8f0' }}>
                        The frontend TypeScript application serves rapidly mutating ABM state space logic alongside entirely local Edge Processing models. 
                        We utilize a completely standalone RAG workflow directly connected to the React UI without legacy datacenter chokepoints.
                    </p>
                    
                    <ul style={{ lineHeight: '1.8', margin: '1.5rem 0', paddingLeft: '1.5rem', color: '#cbd5e1' }}>
                        <li><strong style={{ color: '#fbbf24' }}>Synthetic Generation Pipeline:</strong> The frontend <code>BackendTrainer.tsx</code> portal contains a <code>handleSyntheticFarming()</code> script to manually generate payload histories completely localized to the browser state.</li>
                        <li><strong style={{ color: '#fbbf24' }}>Edge Deep Learning:</strong> We execute PyTorch `.onnx` models natively within the Client architecture using WASM runtime APIs to eliminate `fetch()` round trips completely.</li>
                        <li><strong style={{ color: '#fbbf24' }}>Literature RAG Synthesis:</strong> The `LLMEngine.ts` provides native web LLM abstractions pulling data directly into the React context, generating official clinical JSON schemas locally.</li>
                    </ul>
